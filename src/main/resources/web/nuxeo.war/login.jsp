...
/* Handle alternative auth plugin cookie */
final String alternativeAuthPluginCookieName = "org.nuxeo.auth.plugin.alternative";
final String formAuthPluginName = "FORM_AUTH";
final String removeAlternativeAuthPluginCookieParam = "noAlternativeAuthPlugin";
// Get cookie
Cookie alternativeAuthPluginCookie = null;
Cookie cookies[] = request.getCookies();
if (cookies != null) {
  for (int i = 0; i < cookies.length; i++) {
    if (alternativeAuthPluginCookieName.equals(cookies[i].getName())) {
      alternativeAuthPluginCookie = cookies[i];
      break;
    }
  }
}
...
if (alternativeAuthPluginCookie == null) {
  alternativeAuthPluginCookie = new Cookie(alternativeAuthPluginCookieName, formAuthPluginName);
  // 10 years
  alternativeAuthPluginCookie.setMaxAge(60*60*24*365*10);
  alternativeAuthPluginCookie.setPath(context);
  response.addCookie(alternativeAuthPluginCookie);
}
// Remove and redirect if needed
else if ("true".equals(request.getParameter(removeAlternativeAuthPluginCookieParam))) {
    alternativeAuthPluginCookie.setMaxAge(0);
    alternativeAuthPluginCookie.setValue("");
    alternativeAuthPluginCookie.setPath(context);
    response.addCookie(alternativeAuthPluginCookie);
      ...
      <td align="center">
        ...
        <form method="post" action="nxstartup.faces">
          ...
          <%} %>
            ...
            <tr>
                <td colspan="2">
                  <a href="/nuxeo/login.jsp?noAlternativeAuthPlugin=true">Se connecter en tant qu'interne</a>
                </td>
              </tr>
          </table>
        </form>
